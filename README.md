...
Windows/Linux 배포 파일은 GitHub Actions의 `Desktop Release` workflow로
만들 수 있습니다. macOS 팀 배포용 Release는 Apple Developer Program이 준비될
때까지 제외합니다.

1. GitHub 저장소의 `Actions` 탭에서 `Desktop Release`를 선택합니다.
2. `Run workflow`를 누르고 `app-v0.1.0` 같은 tag를 입력합니다.
3. workflow가 끝나면 draft GitHub Release에 Windows/Linux artifact가
   첨부됩니다.

macOS Release note:
- Apple Developer Program이 없으면 Developer ID signing/notarization을 할 수
  없으므로 GitHub Release에는 macOS DMG를 올리지 않습니다.
- macOS 내부 QA는 로컬 개발 Mac에서 `npm run tauri:build`로 생성된
  `src-tauri/target/release/bundle/macos/GStreamer To Topology.app` 또는
  `src-tauri/target/release/bundle/dmg/*.dmg`를 사용합니다.
- 로컬 내부 QA 산출물은 unsigned/not notarized이므로 팀 배포용 Pass 기준이
  아닙니다. Safari/Chrome으로 다운로드한 DMG에서 `손상됨` 경고가 나오면 정상적인
  Gatekeeper 차단입니다.
- 향후 팀 배포용 macOS Release가 필요하면 Apple Developer Program, Developer ID
  Application 인증서, notarization credential을 준비한 뒤 Release workflow에
  macOS signing/notarization job을 다시 추가합니다.
